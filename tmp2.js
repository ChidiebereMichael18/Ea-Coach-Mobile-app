...
const BookingScreen = ({ route, navigation }) => {
  ...
  const renderSeatStep = () => (
    <View style={{ flex: 1 }}>
      {renderHeader("Select Seats", `${selectedBus?.operator?.name} ΓÇó ${selectedBus?.route?.departureTime}`)}
    ...
    <View style={styles.confirmView}>
      ...
      <View style={styles.ticketCard}>
        <View style={styles.ticketTop}>
           ...
           <Text style={styles.tickRoute}>{searchData.from} ΓåÆ {searchData.to}</Text>
        </View>
